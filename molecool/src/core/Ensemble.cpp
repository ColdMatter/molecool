...
namespace molecool {
	...
	Ensemble::Ensemble(long nParticles, std::array<molecool::Distribution, 6>& dists)
	{
		...
		// need to populate ensemble, this way should be the fastest due to low number of cache misses
		...
		// somewhat slow though, every read results in a cache miss
		for (long i = 0; i < nParticles; ++i) {
			std::valarray<double> vArray = vTarget[std::slice(i, 6, nParticles)];		// (start, size, stride)
			particles[i] = Particle(vArray);
		}
		*/
